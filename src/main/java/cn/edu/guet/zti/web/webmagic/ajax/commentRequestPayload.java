...
public class commentRequestPayload {
    ...
    private static void option( String sightUrlId) throws IOException {
        Connection optionsConnect = Jsoup.connect(COMMENT_AJAX_URL + CID)
                ...
                .referrer(REFERER_URL + sightUrlId)
    ...
    private Connection.Response post(int pagenum, int pagesize,   String sightUrlId) throws IOException {
        option(sightUrlId);
        String viewId = sightUrlId.substring(1, sightUrlId.indexOf(".html"));
        String jsonBody = "{\"pageid\":10650000804,\"viewid\":" + viewId + ",\"tagid\":0,\"pagenum\":"
    ...
    public Connection.Response getComment(int pagenum, int pagesize, String sightUrlId) throws IOException {
        return new commentRequestPayload().post(pagenum, pagesize, sightUrlId);
    }
}
