...
public class SpiderTest {
    ...
    public void crawlAllPlacesAndPersistent() {
        Spider.create(new PlacesPageProcessor())
                .addUrl(Constant.GUANGXI_SIGHTLIST_URL)
                //打印各个地方的信息
...
//                .addPipeline(persistentPlacePipeline)
    ...
    /**
     * 爬取一个景点的详细信息,不包括用户评论，并持久化
     */
    @Test
    public void crawlASightAndPersistent() {
        Spider.create(
//                new SightPageProcessor("longsheng1445120")
                new SightPageProcessor("guilin28")
        )
//                .addUrl("https://you.ctrip.com/sight/longsheng1445120/1476665.html")
                .addUrl("https://you.ctrip.com/sight/guilin28/108723.html")
//                .addUrl("https://you.ctrip.com/sight/guilin28/2888.html")
                .setDownloader(new CustomHttpClientDownloader())
                .addPipeline(new ConsolePipeline())
    ...
    public void crawlAPlaceAllSights() {
        String placeLink = "/sight/lingyun1446254.html";
        Spider.create(new ToSightPageProcessor(UrlUtils.getPlaceUrlId(placeLink), persistentSightPipeline))
...
//                .addPipeline(new ConsolePipeline())
    ...
    public void crawlAllPlacesAllSights() {
        ...
        for (String placeUrlId : placeUrlIdList) {
            placeLink = UrlUtils.getPlaceLinkPrefixSight(placeUrlId);
    ...
    public void checkWhichPlaceNotFoundAndCrawlPersistent() {
        ...
        for (String placeUrlId : placeUrlIdNotFoundList) {
            String placeLink = UrlUtils.getPlaceLinkPrefixSight(placeUrlId);
            Spider.create(new ToSightPageProcessor(placeUrlId, persistentSightPipeline))
                    .addUrl(UrlUtils.getUrl(placeLink))
                    //根据该Spider获取到的景点链接，再创建Spider爬取每一个景点的详细信息
                    .addPipeline(new SightPipeline())
...
//        int beginLine = 7459;
//        String filePath = "C:\\Users\\gnosed\\Desktop\\景点第二次爬取.txt";
        int beginLine = 34;
        String filePath = "C:\\Users\\gnosed\\Desktop\\景点第一次爬取.txt";
        ...
        try {
            ...
            while ((line = bufferedReader.readLine()) != null) {
                if (line.contains(Constant.RUN_TIME_OUT_KEYWORD)) {
...
//                .addPipeline(new ConsolePipeline())
    ...
    public void crawlRequestPayloadComment() throws IOException {
        System.out.println(new commentRequestPayload()
                .getComment(3, 10, "t51452.html", "09031153311214479604"));
    }
